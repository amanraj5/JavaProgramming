...
The brand and grade should be concatenated with :: while printing. eg:- Tesla::A1, where Tesla is the brand and Al
is the grade.
...
public class Solution {
    public static void main(String[] args){
        ...
        for(int i=0;i<n;i++){
            ...
            cars[i] = new Car(id,b,t,p,e);
        }
        String env=sc.nextLine();
        String brand=sc.nextLine();

        int ans1=findTestPassedByEnv(cars,env);
        List<Car> ans2=updateCarGrade(cars,brand);
        ...
        if(ans2 != null){
            for(int i=0;i<ans2.size();i++){
                int rating=(ans2.get(i).getPassed()*100)/ans2.get(i).getTests();
                if(rating>=80){
                    System.out.println(ans2.get(i).getBrand()+"::A1");
                }
                else {
                    System.out.println(ans2.get(i).getBrand() + "::B2");
                }
            }
        }
        else {
    ...
    public static List<Car> updateCarGrade(Car[] cars,String brand){
...
class Car {
    ...
    public Car(int id,String brand,int tests,int passed,String environment){
        this.id=id;
        this.brand=brand;
        this.tests=tests;
        this.passed=passed;
